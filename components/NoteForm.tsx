    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end md:items-center justify-center z-[60] p-0 md:p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-t-[2.5rem] md:rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom-full md:slide-in-from-bottom-4 duration-300 max-h-[95vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
          ...
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">O que está pensando?</label>
            <textarea 
              value={content}
          ...
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-1">Personalizar Cor</label>
          ...
          <div className="pt-4 flex gap-3 pb-4">
            <Button variant="ghost" type="button" className="flex-1 rounded-2xl h-14 font-black" onClick={onCancel}>
              CANCELAR
            </Button>
            <Button variant="primary" type="submit" className="flex-[2] rounded-2xl h-14 font-black shadow-xl shadow-indigo-100">
              SALVAR NOTA
            </Button>
          </div>
        </form>
